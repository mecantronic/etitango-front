        ...
        {({ setFieldValue, touched, errors, values, isSubmitting }) => (
          <Form>
            <Box sx={{ display: {xs: 'none', md:'flex'}, justifyContent: 'space-between', alignItems: 'center', padding: '24px 0px 24px 0px', margin: '0px 20px 0px 20px' }}>
              <Typography sx={{ fontWeight: 600, fontSize: '24px' }}>{t('titleInfoGeneral')}</Typography>
              <Box sx={{ display: 'flex', mr: 1, alignItems: 'center' }}>
                {userIsSuperAdmin ? <>
                  {!enable && !isMobile && (
                    <Typography sx={{ fontWeight: 600, fontSize: '24px', color: 'details.frenchBlue', mr: 1 }}>{selectedEvent?.name}</Typography>
                  )}
                  {enable && !isMobile && (
                      <Field
                        name="name"
                  ...
                  <Button onClick={() => handleEditDataEvent(values)}>
                   {!enable && !isMobile ? <BorderColorIcon sx={{color: 'details.perseanOrange'}}></BorderColorIcon> : <CheckCircleIcon sx={{ width: '38px', height: '38px', color: 'status.success' }}/>}
                  </Button>
                </> : <Typography sx={{ fontWeight: 600, fontSize: '24px', color: 'details.frenchBlue', mr: 1 }}>{selectedEvent?.name}</Typography>}
              </Box>
            </Box>
            <Box sx={{ margin: '0px 20px 0px 20px', backgroundColor: 'greyScale.50', borderRadius: '12px 12px 0px 0px', p: 2 }}>
              <Grid container spacing={2}>
                ...
                <Grid item md={12} sm={12} xs={12}>
                  <Grid container spacing={2}>
                    <Grid item md={6} sm={4} xs={4} lg={4} >
                      <Typography style={{ fontFamily: 'roboto', color: 'details.frenchBlue' }}>
                        {t('label.from')}
                      </Typography>
                      <Grid sx={{  display: 'flex', flexDirection: 'row', justifyContent: 'space-evenly', border: {md: 'none', lg: `1px solid ${theme.palette.details.perseanOrange}`}, alignItems: 'center', borderRadius: 2 }}>
                        ...
                        <Box sx={{ display: {md:'flex', lg: 'inherit'}, justifyContent: {md: 'center', lg: 'inherit'}, width: {md:'30%', lg: 'inherit'}}}>
                          <Typography style={{ fontFamily: 'roboto', color: 'details.frenchBlue', fontSize: '16px', width: 'max-content' }}>{t('label.time')}</Typography>
                        </Box>
                    ...
                    <Grid item md={6} sm={4} xs={4} lg={4}>
                      ...
                      <Box sx={{  display: 'flex', flexDirection: 'row', justifyContent: 'space-evenly',border: {md: 'none', lg: `1px solid ${theme.palette.details.perseanOrange}`}, alignItems: 'center', borderRadius: 2 }}>
                        <Box>
                          <ETIDatePicker
                            textFieldProps={{ fullWidth: true }}
                            fieldName="dateEnd"
                ...
                <Box sx={{borderBottom: `1px solid ${theme.palette.greyScale[400]}`, width: '100%', ml: 2, mt: 2}}></Box> 
                <Grid item md={12} sm={12} xs={12}>
                   <Grid container spacing={2}>
                   <Grid item md={12} sm={12} xs={12}>
                ...
                <Grid item md={12} sm={12} xs={12}>
                  <Grid container gap={2} sx={{ border: `1.5px solid ${theme.palette.details.perseanOrange}`, borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }} >
                    ...
                    {admins.length === 0 ? (
                        ...
                        admins.map((admin: any, index) => (
                          <Chip key={index} label={admin.name} {...(enable ? { onDelete: () => handleDelete(admin.email) } : {})} variant="outlined" sx={{ m: 1, borderRadius: '8px', color: 'principal.secondary', fontFamily: 'Roboto', fontWeight: 500, fontSize: '14px' }} />
                        ))
                      )}
                    </Box>
                    {enable && !isMobile && 
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', margin: '20px' }}>
                  {isMobile ?  <Button onClick={() => handleEditDataEvent(values)}>
                        {!enable  ? <BorderColorIcon sx={{ color: 'details.perseanOrange'}}></BorderColorIcon> : <CheckCircleIcon sx={{ width: '38px', height: '38px', color: 'status.success'}}/>}
                        </Button>
                      :
                        <Button type='submit' disabled={isSubmitting} sx={{ width: '115px', padding: '12px, 32px, 12px, 32px', borderRadius: '25px', backgroundColor: 'principal.secondary', height: '44px', '&:hover': { backgroundColor: 'principal.secondary' } }}>
                          {isLoading ? <CircularProgress sx={{ color: 'background.white' }} size={30} /> : <><Typography sx={{ color: 'greyScale.50', fontWeight: 500, fontSize: '14px', lineHeight: '20px' }}>{showSuccessImage ? 'Guardado' : 'Guardar'}</Typography>{showSuccessImage && <img src={'/img/icon/Vector.svg'} height={15} width={15} style={{ marginLeft: '10px' }} />}</>}
                        </Button>}
              </Box>
              <Box sx={{display: {xs: 'flex', md: 'none'}, borderBottom: `1px solid ${theme.palette.greyScale[400]}`, width: '100%', mt: 2}}></Box> 
            </Box>
          </Form>
        )}
      </Formik>
